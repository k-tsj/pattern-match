...
== Example
 ...
 ## (A)
 match([0, [1, 2, 3, 4]]) do
   with(_[a, _[b, *c, d]]) do # Same as `Array.(a, Array.(b, *c, d))'
     p [a, b, c, d] #=> [0, 1, [2, 3], 4]
   end
 end

 ## (B)
 # quantifier: ___, ___?, __n, __n?
 match([[0, 1], [2, 3]]) do
   with(_[_[a, b], ___]) do
     p [a, b] #=> [[0, 2], [1, 3]]
   end
 ...
 match([[0, :a], [1]]) do
   with(_[_[a & Fixnum, ___, *b], ___]) do
     p [a, b] #=> [[[0], [1]], [[:a], []]]
   end
 end

 match([0, 1, 2, 3, 4]) do
   with(_[*_, a & Fixnum, ___, *_], guard { a.inject(:*) == 6 }) do
     p a #=> [2, 3]
   end
 end
 ...
 ## (E)
 match({a: 0, b: 1, c: 2}) do
   with(Hash.(:a, :b, c: 2)) do
     p [a, b] #=> [0, 1]
   end
 end
